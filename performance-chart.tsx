...
const generateFPSData = (): PerformanceData[] => {
  ...
  const values = [1, 8, 2, 0, 5, 15, 12, 7, 14, 7, 10, 8, 12, 15, 9, 7, 10, 8, 5, 2, 10, 7, 3, 10, 5, 3, 4, 5, 4, 3]
    ...
    <div className="relative">
      <div className="text-gray-300 text-sm font-medium mb-2 flex items-center">
        ...
        <svg className="ml-1" width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M12 7v6M12 16v1" stroke="#666" strokeWidth="2" strokeLinecap="round" />
        </svg>
      </div>
      <svg width={width} height={height}>
        ...
        <defs>
          {/* Create a grid pattern for the texture */}
          <pattern id={patternId} width="4" height="4" patternUnits="userSpaceOnUse">
            <rect width="4" height="4" fill="#121212" />
            <rect width="1" height="1" fill={color} fillOpacity="0.3" />
          </pattern>

          {/* Create a gradient for the area */}
          <LinearGradient id={gradientId} from={color} to={color} fromOpacity={0.3} toOpacity={0.05} />

          {/* Create a mask that combines the pattern and gradient */}
          <mask id={maskId}>
            <rect width={innerWidth} height={innerHeight} fill="url(#${gradientId})" />
          </mask>
        </defs>

        <Group left={margin.left} top={margin.top}>
          <GridColumns scale={timeScale} height={innerHeight} stroke="#222" strokeOpacity={0.8} numTicks={5} />

          <GridRows scale={valueScale} width={innerWidth} stroke="#222" strokeOpacity={0.8} numTicks={5} />
      ...
      {tooltipData && tooltipLeft != null && tooltipTop != null && (
        <Tooltip top={tooltipTop - 40} left={tooltipLeft} style={tooltipStyles}>
          <div className="text-xs">
            <div className="font-medium">{formatTime(getTime(tooltipData)).toLowerCase()}</div>
            <div>
...
const generateBitRateData = (): PerformanceData[] => {
  ...
  const values = [
    0, 200, 100, 50, 100, 300, 500, 600, 400, 200, 600, 800, 400, 1000, 600, 200, 100, 400, 600, 1000, 800, 400, 200,
    600, 400, 200, 300, 200, 150, 100,
  ]
